...
public class CadastroFotografo {
	...
	public void criar() {
		try {
			conn.conectarBD();
			String sql = "insert into cadastro_fotografo (nome, cpf, telefone, endereco, numero, complemento, bairro, cep, cidade, estado, estilofoto) \r\n  values(?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean ler() {
		...
		try {
			conn.conectarBD();
			String sql = "select * from cadastro_fotografo where id = ?";
	...
	public void alterar() {
		try {
			conn.conectarBD();
			String sql = "update cadastro_fotografo set nome =?,cpf =?,telefone =?,endereco =?, numero=?, complemento=?, bairro=?, cep=?, cidade=?, estado=?, estilofoto=? where id = ?";
			PreparedStatement stmt = conn.getConector().prepareStatement(sql);
			stmt.setString(1, nome);
			stmt.setString(2, cpf);
			stmt.setString(3, telefone);
	...
	public void apagar() {
		try {
			conn.conectarBD();
			String sql = "delete from cadastro_fotografo where id = ?";
			PreparedStatement stmt = conn.getConector().prepareStatement(sql);
	...
	// Tabela
	public DefaultTableModel listaFotografo() throws SQLException {
		DefaultTableModel ret = null;
		conn.conectarBD();
		String sql = "select * from cadastro_fotografo order by id";
		PreparedStatement stmt = conn.getConector().prepareStatement(sql);
	...
	public String getEstilofoto() {
		return estilofoto;
	}

	public void setEstilofoto(String estilofoto) {
		this.estilofoto = estilofoto;
	}
